...
import { useTaskEdit, ValidationError } from '../composables/useTaskEdit'
...
async function onSubmit() {
  ...
  try {
    await handleSubmit(props.task.id)
...
function destLabel(): string {
  if (!props.task?.dest_endpoint_id) return ''
  const ep = endpointOptions.value.find((e) => e.value === props.task!.dest_endpoint_id)
  return ep?.label || props.task!.dest_endpoint_id
}
</script>
  ...
  <NModal :show="show" :mask-closable="false" @update:show="emit('update:show', $event)">
    <div class="bg-white rounded-xl shadow-2xl w-[560px]">
      ...
      <div class="flex items-center justify-between px-6 py-5 border-b border-gray-100">
        <span class="text-lg font-semibold text-gray-900">编辑任务</span>
      ...
      <div class="px-6 py-5">
        <NForm :label-width="120" label-placement="left">
          ...
          <NFormItem v-if="needDest" label="目标端点">
            <NInput :value="destLabel()" disabled />
          </NFormItem>
