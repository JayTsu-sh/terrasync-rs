...
pub async fn accept(listen_addr: SocketAddr) -> Result<(QuicReceiverTransport, quinn::Endpoint, Vec<u8>)> {
    ...
    let (certs, key) =
        cert::generate_self_signed(&[]).map_err(|e| TransportError::RecvFailed(format!("cert: {}", e)))?;

    // 在 certs 被 build_server_config 消费前，先保存 DER 字节
    let cert_der: Vec<u8> = certs[0].as_ref().to_vec();

    let server_config = cert::build_server_config(certs, key)
        .map_err(|e| TransportError::RecvFailed(format!("server config: {}", e)))?;
