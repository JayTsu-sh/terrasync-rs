...
```bash
ssh root@{SOURCE_IP} 'sudo find {SOURCE_NFS_EXPORT} -mindepth 1 -maxdepth 1 -exec rm -rf {} + && echo "source cleaned"'
```
...
```bash
ssh root@{DEST_IP} 'sudo find {DEST_NFS_EXPORT} -mindepth 1 -maxdepth 1 -exec rm -rf {} + && echo "dest cleaned"'
```
