...
use crate::{
    DataChunk, DeleteDirIterator, EntryEnum, Result, ResumeContext, WalkDirAsyncIterator, WalkDirAsyncIterator2,
};
...
impl StorageEnum {
    ...
    pub async fn copy_file_resumable(
        from: &StorageEnum, to: &StorageEnum, entry: &EntryEnum, qos: Option<QosManager>, enable_integrity_check: bool,
        is_source_reserved: bool, bytes_counter: Option<Arc<AtomicU64>>, resume: ResumeContext,
    ) -> Result<()> {
        let size = entry.get_size();
        let ResumeContext {
            part_relative_path,
        ...
        let read_task = tokio::spawn(async move {
            match (&from_c, &entry_r) {
                (StorageEnum::Local(s), EntryEnum::NAS(e)) => {
                    s.read_data_intervals(tx, &e.relative_path, &intervals, qos_r).await
                }
                (StorageEnum::NFS(s), EntryEnum::NAS(e)) => {
                    s.read_data_intervals(tx, &e.relative_path, &intervals, qos_r).await
                }
        ...
        let write_task = tokio::spawn(async move {
            match (&to_c, &entry_w) {
                (StorageEnum::Local(s), EntryEnum::NAS(e)) => {
                    s.write_data_resumable(rx, &part, e.uid, e.gid, Some(e.mode), bytes_counter_w, on_committed)
                        .await
                }
                (StorageEnum::NFS(s), EntryEnum::NAS(e)) => {
                    s.write_data_resumable(rx, &part, e.uid, e.gid, Some(e.mode), bytes_counter_w, on_committed)
        ...
        to.rename(&part_relative_path, entry.get_relative_path()).await?;
...
fn deserialize_nfs_acl(data: &[u8]) -> Result<nfs_rs::Acl> {
    ...
    for _ in 0..count {
        let ace_type = match read_u32_le(&mut cursor, "ACE type")? {
            ...
            v => {
                return Err(StorageError::OperationError(format!("unknown ACE type: {v}")));
            }
        };
...
/// S3 无目录概念，该参数对其无效果。
pub async fn create_storage(path: &str, block_size: Option<u64>, ensure_dir: bool) -> Result<StorageEnum> {
