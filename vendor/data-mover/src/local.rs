...
impl LocalStorage {
    ...
    pub(crate) async fn read_data_intervals(
        &self, tx: Sender<DataChunk>, relative_path: &Path, intervals: &[(u64, u64)], qos: Option<QosManager>,
    ) -> Result<()> {
    ...
    pub(crate) async fn write_data_resumable(
        &self, rx: tokio::sync::mpsc::Receiver<DataChunk>, part_path: &Path, uid: Option<u32>, gid: Option<u32>,
        mode: Option<u32>, bytes_counter: Option<Arc<AtomicU64>>, on_committed: crate::CommitCallback,
    ) -> Result<()> {
...
pub fn create_local_storage(path: &str, block_size: Option<u64>, ensure_dir: bool) -> Result<StorageEnum> {
    if ensure_dir {
